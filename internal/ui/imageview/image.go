...
// Unicode block element runes (see https://en.wikipedia.org/wiki/Block_Elements)
const (
	BlockLowerOneEighthBlock            = '\u2581' // LOWER ONE EIGHTH BLOCK
	BlockLowerOneQuarterBlock           = '\u2582' // LOWER ONE QUARTER BLOCK
	BlockLowerThreeEighthsBlock         = '\u2583' // LOWER THREE EIGHTHS BLOCK
	BlockLowerHalfBlock                 = '\u2584' // LOWER HALF BLOCK
	BlockLowerFiveEighthsBlock          = '\u2585' // LOWER FIVE EIGHTHS BLOCK
	BlockLowerThreeQuartersBlock        = '\u2586' // LOWER THREE QUARTERS BLOCK
	BlockLowerSevenEighthsBlock         = '\u2587' // LOWER SEVEN EIGHTHS BLOCK
	BlockFullBlock                      = '\u2588' // FULL BLOCK
	BlockLeftSevenEighthsBlock          = '\u2589' // LEFT SEVEN EIGHTHS BLOCK
	BlockLeftThreeQuartersBlock         = '\u258A' // LEFT THREE QUARTERS BLOCK
	BlockLeftFiveEighthsBlock           = '\u258B' // LEFT FIVE EIGHTHS BLOCK
	BlockLeftHalfBlock                  = '\u258C' // LEFT HALF BLOCK
	BlockLeftThreeEighthsBlock          = '\u258D' // LEFT THREE EIGHTHS BLOCK
	BlockLeftOneQuarterBlock            = '\u258E' // LEFT ONE QUARTER BLOCK
	BlockLeftOneEighthBlock             = '\u258F' // LEFT ONE EIGHTH BLOCK
	BlockQuadrantLowerLeft              = '\u2596' // QUADRANT LOWER LEFT
	BlockQuadrantLowerRight             = '\u2597' // QUADRANT LOWER RIGHT
	BlockQuadrantUpperLeft              = '\u2598' // QUADRANT UPPER LEFT
	BlockQuadrantUpperRight             = '\u259D' // QUADRANT UPPER RIGHT
	BlockQuadrantUpperLeftAndLowerRight = '\u259A' // QUADRANT UPPER LEFT AND LOWER RIGHT
	BlockLightShade                     = '\u2591' // LIGHT SHADE
	BlockMediumShade                    = '\u2592' // MEDIUM SHADE
	BlockDarkShade                      = '\u2593' // DARK SHADE
)
...
type Image struct {
	*tview.Box
	...
	// Horizontal and vertical alignment, one of the Alignment constants.
	alignHorizontal, alignVertical tview.Alignment
...
func NewImage() *Image {
	i := &Image{
		...
		aspectRatio:     0.5,
		alignHorizontal: tview.AlignmentCenter,
		alignVertical:   tview.AlignmentCenter,
	}
	return i
}
...
// widget's space. The possible values are tview.AlignmentTop, tview.AlignmentCenter, tview.AlignmentBottom for vertical alignment and tview.AlignmentLeft, tview.AlignmentCenter, tview.AlignmentRight for horizontal alignment. The default is tview.AlignmentCenter for both.
func (i *Image) SetAlign(vertical, horizontal tview.Alignment) *Image {
...
// SetDisabled is a no-op for Image (always read-only).
func (i *Image) SetDisabled(disabled bool) *Image {
	return i
}
...
// SetFormAttributes is a no-op for Image.
func (i *Image) SetFormAttributes(labelWidth int, labelColor, bgColor, fieldTextColor, fieldBgColor tcell.Color) *Image {
...
func (i *Image) SetFinishedFunc(handler func(key tcell.Key)) *Image {
	i.finished = handler
	return i
}

// Focus is called when this model receives focus.
func (i *Image) Focus(delegate func(m tview.Model)) {
	if i.finished != nil {
		i.finished(-1)
		return
	}
...
func (i *Image) render() {
	...
	_, _, innerWidth, innerHeight := i.InnerRect()
	if i.labelWidth > 0 {
		innerWidth -= i.labelWidth
	} else {
		// Use a minimal string width calculation (rune count)
		innerWidth -= len([]rune(i.label))
	}
	if innerWidth <= 0 {
		i.pixels = nil
		return
...
func (i *Image) Draw(screen tcell.Screen) {
	...
	// Draw label (minimal implementation: left-aligned, no style parsing)
	viewX, viewY, viewWidth, viewHeight := i.InnerRect()
	labelRunes := []rune(i.label)
	labelWidth := len(labelRunes)
	if i.labelWidth > 0 && i.labelWidth < viewWidth {
		labelWidth = i.labelWidth
	} else if labelWidth > viewWidth {
		labelWidth = viewWidth
	}
	for iLabel := 0; iLabel < labelWidth && iLabel < len(labelRunes); iLabel++ {
		screen.SetContent(viewX+iLabel, viewY, labelRunes[iLabel], nil, i.labelStyle)
	}
	viewX += labelWidth
	...
	if i.alignHorizontal == tview.AlignmentCenter {
		x += (viewWidth - width) / 2
	} else if i.alignHorizontal == tview.AlignmentRight {
		x += viewWidth - width
	}
	if i.alignVertical == tview.AlignmentCenter {
		y += (viewHeight - height) / 2
	}
	// else: default is top alignment
...
// RenderLines renders the image into terminal cell lines that can be embedded
// in other widgets (for example, chat message previews).
func (i *Image) RenderLines(width, height int) []tview.Line {
	if width <= 0 || height <= 0 {
		return nil
	}
	...
	i.render()
	if i.lastWidth <= 0 || i.lastHeight <= 0 || len(i.pixels) == 0 {
		return nil
	}

	lines := make([]tview.Line, 0, i.lastHeight)
	for row := 0; row < i.lastHeight; row++ {
		line := make(tview.Line, 0, i.lastWidth)
		var (
		...
		flush := func() {
			...
			line = append(line, tview.NewSegment(buf.String(), currentStyle))
			buf.Reset()
		}

		for col := 0; col < i.lastWidth; col++ {
			px := i.pixels[row*i.lastWidth+col]
			if !hasStyle {
				currentStyle = px.style
				hasStyle = true
			}
			if px.style != currentStyle {
				flush()
				currentStyle = px.style
			}
			buf.WriteRune(px.element)
		}
		flush()
